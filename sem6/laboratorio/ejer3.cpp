...
int buscarProductoID(const char* nombreArchivo, int idBuscado); //ok
...
int main(){
    registrarProducto("inventario.dat"); // Registra uno
    mostrarProductos("inventario.dat");  // Debería mostrar el que acabas de crear
    return 0;
}
...
void mostrarProductos(const char* nombreArchivo){
    ...
    Producto p;
    cout<<"__LISTA DE PRODUCTOS__"<<endl;
    while(archivo.read((char*)&p, sizeof(Producto))){
        if(p.activo){
            cout<<"ID: "<<p.id<<
            "| Nombre: "<<p.nombre<<
...
void eliminarProducto(const char* nombreArchivo, int id){
    ...
    archivo.write((char*)&p, sizeof(Producto));
    archivo.close();
    cout<<"Producto co id "<<id<<" eliminado"<<endl;
}
